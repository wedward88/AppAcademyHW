...
def pulp_fiction_actors
  ...
  Actor.select('actors.id, actors.name').joins(:castings).joins(:movies).where("title = 'Pulp Fiction' ").group('actors.id')
end
...
def uma_movies
  ...
  Movie.select("movies.id, movies.title, movies.yr").joins(:castings).joins(:actors).where("name = 'Uma Thurman'").group('movies.id').order('yr ASC')
end
